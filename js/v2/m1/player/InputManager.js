export class InputManager {
    move = { x: 0, y: 0 };
    pendingLook = { dx: 0, dy: 0 };
    keys = new Set();
    actionHandlers = new Map();
    cleanup = [];
    attach(root = document) {
        const keydown = (event) => {
            this.keys.add(event.code);
            this.recomputeKeyboardMove();
            if (!event.repeat) {
                if (event.code === "KeyE")
                    this.emitAction("interact");
                if (event.code === "Tab") {
                    event.preventDefault();
                    this.emitAction("os");
                }
                if (event.code === "Escape")
                    this.emitAction("pause");
            }
        };
        const keyup = (event) => {
            this.keys.delete(event.code);
            this.recomputeKeyboardMove();
        };
        window.addEventListener("keydown", keydown);
        window.addEventListener("keyup", keyup);
        this.cleanup.push(() => window.removeEventListener("keydown", keydown));
        this.cleanup.push(() => window.removeEventListener("keyup", keyup));
        const movePad = root.querySelector("[data-m0-move]");
        const moveKnob = movePad?.querySelector("[data-m0-move-knob]") ?? null;
        if (movePad)
            this.bindMovePad(movePad, moveKnob);
        const lookPad = root.querySelector("[data-m0-look]");
        if (lookPad)
            this.bindLookPad(lookPad);
        root.querySelectorAll("[data-m0-action]").forEach(button => {
            const handler = (event) => {
                event.preventDefault();
                this.emitAction(button.dataset.m0Action ?? "");
            };
            button.addEventListener("pointerdown", handler);
            this.cleanup.push(() => button.removeEventListener("pointerdown", handler));
        });
    }
    onAction(action, handler) {
        const bucket = this.actionHandlers.get(action) ?? new Set();
        bucket.add(handler);
        this.actionHandlers.set(action, bucket);
        return () => bucket.delete(handler);
    }
    consumeLookDelta() {
        const value = { ...this.pendingLook };
        this.pendingLook.dx = 0;
        this.pendingLook.dy = 0;
        return value;
    }
    destroy() {
        for (const fn of this.cleanup.splice(0))
            fn();
        this.actionHandlers.clear();
    }
    emitAction(action) {
        if (!action)
            return;
        for (const handler of this.actionHandlers.get(action) ?? [])
            handler();
    }
    recomputeKeyboardMove() {
        const x = Number(this.keys.has("KeyD") || this.keys.has("ArrowRight")) - Number(this.keys.has("KeyA") || this.keys.has("ArrowLeft"));
        const y = Number(this.keys.has("KeyS") || this.keys.has("ArrowDown")) - Number(this.keys.has("KeyW") || this.keys.has("ArrowUp"));
        const len = Math.hypot(x, y) || 1;
        this.move.x = x / len;
        this.move.y = y / len;
    }
    bindMovePad(pad, knob) {
        let pointerId = null;
        const update = (event) => {
            const rect = pad.getBoundingClientRect();
            const cx = rect.left + rect.width / 2;
            const cy = rect.top + rect.height / 2;
            let x = (event.clientX - cx) / (rect.width * 0.33);
            let y = (event.clientY - cy) / (rect.height * 0.33);
            const len = Math.hypot(x, y) || 1;
            if (len > 1) {
                x /= len;
                y /= len;
            }
            this.move.x = x;
            this.move.y = y;
            if (knob)
                knob.style.transform = `translate(calc(-50% + ${x * 28}px), calc(-50% + ${y * 28}px))`;
        };
        const down = (event) => {
            if (pointerId !== null)
                return;
            pointerId = event.pointerId;
            pad.setPointerCapture(pointerId);
            update(event);
        };
        const move = (event) => { if (event.pointerId === pointerId)
            update(event); };
        const end = (event) => {
            if (event.pointerId !== pointerId)
                return;
            pointerId = null;
            this.move.x = 0;
            this.move.y = 0;
            if (knob)
                knob.style.transform = "translate(-50%, -50%)";
        };
        pad.addEventListener("pointerdown", down);
        pad.addEventListener("pointermove", move);
        pad.addEventListener("pointerup", end);
        pad.addEventListener("pointercancel", end);
        this.cleanup.push(() => { pad.removeEventListener("pointerdown", down); pad.removeEventListener("pointermove", move); pad.removeEventListener("pointerup", end); pad.removeEventListener("pointercancel", end); });
    }
    bindLookPad(pad) {
        const points = new Map();
        const down = (event) => {
            pad.setPointerCapture(event.pointerId);
            points.set(event.pointerId, { x: event.clientX, y: event.clientY });
        };
        const move = (event) => {
            const previous = points.get(event.pointerId);
            if (!previous)
                return;
            this.pendingLook.dx += event.clientX - previous.x;
            this.pendingLook.dy += event.clientY - previous.y;
            points.set(event.pointerId, { x: event.clientX, y: event.clientY });
        };
        const end = (event) => points.delete(event.pointerId);
        pad.addEventListener("pointerdown", down);
        pad.addEventListener("pointermove", move);
        pad.addEventListener("pointerup", end);
        pad.addEventListener("pointercancel", end);
        this.cleanup.push(() => { pad.removeEventListener("pointerdown", down); pad.removeEventListener("pointermove", move); pad.removeEventListener("pointerup", end); pad.removeEventListener("pointercancel", end); });
    }
}
