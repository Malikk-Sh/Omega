...
export function upgradeStateForBackup03(state: OmegaGameState): void {
  ...
  const laterSnapshotScene = /^(?:backup_\d+_\d+|memory_[a-z0-9_]+)$/.test(incomingScene) && !resumeBackup03;
  if (!resumeBackup03 && !laterSnapshotScene) state.world.activeScene = HOME_SCENE;
